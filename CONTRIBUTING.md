...
- **On merge to main:** The [Release version](.github/workflows/release-version.yml) workflow runs [semantic-release](https://semantic-release.gitbook.io/) on Node 22 (required by semantic-release@25). It analyzes new commits, bumps `package.json`, **publishes to npm**, then pushes the version commit and tag (e.g. `v1.2.0`). If you also use [Verify release tag](.github/workflows/verify-release-tag.yml), a tag push runs tests and verifies the tag matches `package.json` (publish is not duplicated there).
