...
7. `.config/ai/rules/theme-color-modes.mdc` — discover `color-schema-*`, brand classes, size overrides, and atoms from `_styles/`; optional brand on `<html>`; section tone; semantic roles only
...
4. **Section file** — Semantic HTML, section-scoped CSS classes, `{% schema %}` with valid JSON. Include translation keys for all user-facing text. Grep `_styles/` first. Brand on `<html>` only if this theme ships a brand class — never on the section. Apply the section tone from the intersection of Figma modes and this theme’s `color-schema-*` (some themes ship `color-schema-accent-1|2|3` — a valid discovered pattern, not “older only”). Children use only semantic roles from `_styles/` (`theme-color-modes.mdc`). Buttons/inputs: existing atoms, not invented utilities. No arbitrary hex. Do not assume stock Tailwind `max-w-*` if `_styles/02_base/*sizes*` overrides them.
