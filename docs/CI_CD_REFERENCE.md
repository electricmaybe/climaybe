...
- **PR preview (`pr-update.yml`):** A first job applies path filters so **share/rename/comment** run only when changes touch theme-facing paths: `assets/`, `blocks/`, `config/`, `layout/`, `locales/`, `sections/`, `snippets/`, `templates/`, `_scripts/`, `_styles/`, `shopify.theme.toml`, or `stores/**` (multi-store). Edits confined to docs, `.github/`, Cursor/AI rules, README, etc. do not redeploy preview. **`pr-close.yml`** is unchanged so preview themes are still removed when the PR closes.
