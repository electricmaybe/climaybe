...
- **Non-staging to main** (hotfix backports, direct commits): **Patch** bump only. Not at commit time; **nightly workflow** at 02:00 US Eastern collects commits since latest tag, ignores no-op/empty-tree commits, generates AI changelog, and creates one patch tag.
...
- **Main → staging-<alias>**: After any push to main (except pure store-sync commits), **main-to-staging-stores** merges main into each `staging-<alias>` so store branches stay in sync: **version bumps**, **hotfixes**, and normal staging→main releases. If a staging branch already has the same git tree as main, that sync is skipped as a no-op. Version-bump commits are pushed by the workflow using `GITHUB_TOKEN`, which does not trigger push-based workflows; therefore **main-to-staging-stores** also runs when **Post-Merge Tag** or **Nightly Hotfix Tag** complete successfully, so the new version reaches all `staging-<alias>`. For **hotfix-backport** commits, only a `staging-<alias> → main` source skips syncing back into the same `staging-<alias>` branch. If the source is `live-<alias> → main`, that same store’s `staging-<alias>` branch is still synced so staging receives the live hotfix too. Only purely store-sync commits are skipped to avoid loops.
...
- **Select store**: `climaybe theme switch <alias>` or `climaybe switch <alias>` (e.g. `climaybe switch voldt-staging`). Copies `stores/<alias>/` JSONs to repo root and updates `default_store` for local `shopify theme dev` / `climaybe serve`. On a `staging-<alias>` or `live-<alias>` branch, `climaybe serve` / `serve:shopify` still pass that branch’s store domain to Shopify as `--store` (even if `default_store` differs), matching CI preview behavior.
...
1. **Pushes a development theme** via `shopify theme push --development` using Dev Dashboard / agency app client credentials (or legacy Theme Access).
...
5. **Cleans up** the development theme after profiling (even if profiling fails).

Required secrets: `SHOPIFY_STORE_URL` and Shopify auth (`SHOP_CLIENT_ID`+`SHOP_CLIENT_SECRET` preferred). When either is missing, the workflow skips gracefully (no failure).
...
- **Path filters (Actions usage):** The reusable build uses `dorny/paths-filter` so **`climaybe build-scripts` runs when `_scripts/**/*.js` or `assets/**/*.js` changed** (and `_scripts/*.js` entrypoints exist). Edits under `assets/*.js` are usually the compiled output (including from this workflow); matching `assets/**/*.js` lets manual or out-of-band changes to bundles trigger a script rebuild. Pushes whose only change is the workflow’s own `chore(assets): update compiled javascript and css` commit from **`github-actions[bot]`** skip the build job to avoid redundant runs. On branches **`live-<alias>`**, any push where **`github.actor`** contains **`[bot]`** (e.g. `github-actions[bot]`, `dependabot[bot]`) skips the entire **Build Pipeline** job so automated commits do not run script/Tailwind compilation. **`climaybe build` (Tailwind) runs when** `**/*.liquid`, `**/*.css`, `_scripts/**/*.js`, or `tailwind.config.*` / `postcss.config.*` changed (and `_styles/main.css` and/or `_styles/critical.css` exists); compiled `assets/**/*.js` alone does **not** force Tailwind (use the script build for bundle changes). If neither filter matches, the job skips `npm ci` and build steps; **Lighthouse** runs only when the build job actually ran (`build_ran`), **secrets are set**, and the branch is exactly **`staging`** (not `staging-*`, `main`, `live-*`, or feature branches). **`build-pipeline.yml`** also ignores pushes that touch only docs, Markdown, `.github/`, `.cursor/`, `.config/`, `.windsurf/`, `.clinerules/`, `AGENTS.md`, `CLAUDE.md`, `.claude/`, `.eser/`, or common readme/license filenames so those edits do not start the pipeline.
- **PR preview (`pr-update.yml`):** Path filters match theme-facing paths (same as before). **Skip preview (no theme push):** add the PR label **`skip-preview`**, or put **`[skip-preview]`** in the PR title, PR body, or head commit message. Label add/remove re-runs the gate (other labels are ignored). When skipped, publish/comment jobs do not run; **`pr-close`** still cleans leftover preview themes. Previews use **`shopify theme push --development --development-context climaybe-pr-<n>`** (hidden from the main theme library). Comment **`/redeploy`** on the PR (`pr-preview-redeploy.yml`) to refresh after ~7 days of inactivity. **Multi-store:** if `climaybe.config.json` has more than one store and the PR **source branch** (`pull_request.head.ref`) is **not** `staging-<alias>` or `live-<alias>`, the workflow runs **push** in a **matrix over every store** and posts **one** PR comment with preview and editor preview links per store (`{alias} preview` · `{alias} editor preview`) (via `preview-fragment-*` artifacts). If the source branch **is** `staging-<alias>` or `live-<alias>`, only that store is used. Before push, each publish job overlays that store’s JSON from `stores/<alias>/` onto the repo root (`config/`, `templates/`, `sections/` — same rules as `climaybe switch` / `stores-to-root`), so previews use the correct per-store template JSON rather than whatever happens to be at root on the PR branch. The PR comment is **upserted** (single bot comment) and older bot preview comments are removed. **`reusable-cleanup-themes`:** `cleanup_mode: by_pr` deletes themes matching `climaybe-pr-<n>` (and legacy names ending with `-PR{padded}`) for the current PR. `cleanup_mode: orphan_pr` deletes those when PR `n` is not open — used by **`cleanup-orphan-preview-themes.yml`**. **`pr-close.yml`:** uses the same store matrix rule as `pr-update` and deletes this PR’s preview themes on each store; a follow-up job sums **deleted** counts from small artifacts for the PR comment.
- **Lighthouse + preview Shopify auth:** Prefer Dev Dashboard / agency app credentials (`SHOP_CLIENT_ID` + `SHOP_CLIENT_SECRET`) with Admin API scopes `write_themes` (and `read_products` for Lighthouse), installed on the CI store(s). CI uses the client credentials grant. Legacy `SHOPIFY_THEME_ACCESS_TOKEN` / `SHOP_ACCESS_TOKEN` still work. `SHOP_PASSWORD` stays a manual secret (Admin API does not return storefront passwords). Also required for Lighthouse: `LHCI_GITHUB_APP_TOKEN`. When auth or those secrets are missing, jobs skip gracefully.
- If `_scripts/*.js` or CSS entrypoints (`_styles/main.css`, `_styles/critical.css`) are missing, the build workflow **skips** the missing step(s) and continues (so builds don’t block other workflows on repos that don’t use these entrypoints yet).
...
- Script builds write readable bundles to `assets/*.js` (use `climaybe build-scripts --minify` locally or in a custom job if you want minified output in-repo).
...
- **No-op guard**: If `origin/main` and the source store branch have the same git tree, the backport is skipped (no empty merge commit).
- **Default vs non-default store**: The **default store** is from `config.default_store` (or first in `config.stores`). Hotfix from the **default** store: root and code changes go to main as usual. Hotfix from a **non-default** store: only changes under **`stores/<alias>/`** are brought to main; detection uses a tree diff on `stores/<alias>/` so root-to-stores sync commits can backport immediately. After merge, root sync files (`config/`, `templates/`, `sections/`) are restored from main so store-specific root JSON does not overwrite main (avoids conflicts).
