...
|--------|------|---------|--------------|
| main-to-staging-<store> | `main-to-staging-stores.yml` | Push to `main`; or **workflow_run** when Post-Merge Tag / Nightly Hotfix Tag complete | Merges main into each `staging-<alias>` (local merge + push). Root JSONs ignored (restored from `stores/<alias>/`). Skips only on pure store-sync. Runs after version-bump workflows so the version-bump commit (pushed with GITHUB_TOKEN) reaches staging. For hotfix-backport: if source is `staging-<alias>`, that same staging branch is skipped; if source is `live-<alias>`, `staging-<alias>` is also synced. |
