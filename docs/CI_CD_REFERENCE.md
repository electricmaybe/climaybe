...
- **PR preview (`pr-update.yml`):** Path filters match theme-facing paths (same as before). **Multi-store:** if `climaybe.config.json` has more than one store and the PR **source branch** (`pull_request.head.ref`) is **not** `staging-<alias>` or `live-<alias>`, the workflow runs **share + rename + cleanup** in a **matrix over every store** and posts **one** PR comment with preview/customize links per store (via `preview-fragment-*` artifacts). If the source branch **is** `staging-<alias>` or `live-<alias>`, only that store is used. **`reusable-cleanup-themes`:** `cleanup_mode: by_pr` deletes themes whose name ends with `-PR{padded}` for the current PR (used before re-publishing). `cleanup_mode: orphan_pr` lists themes ending with `-PR<n>`, compares `n` to **open** PRs via `gh pr list` (limit 1000), and deletes themes whose PR is not open — used by **`cleanup-orphan-preview-themes.yml`** (weekly + `workflow_dispatch`). **`pr-close.yml`:** uses the same store matrix rule as `pr-update` and deletes this PR’s preview themes on each store; a follow-up job sums **deleted** counts from small artifacts for the PR comment.
