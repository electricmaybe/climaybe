...
- **PR preview (`pr-update.yml`):** Path filters match theme-facing paths (same as before). **Skip preview (no theme share/push):** add the PR label **`skip-preview`**, or put **`[skip-preview]`** in the PR title, PR body, or head commit message. Label add/remove re-runs the gate (other labels are ignored). When skipped, publish/comment jobs do not run; **`pr-close`** still cleans any leftover `-PR<n>` themes. **Multi-store:** if `climaybe.config.json` has more than one store and the PR **source branch** (`pull_request.head.ref`) is **not** `staging-<alias>` or `live-<alias>`, the workflow runs **share + rename + cleanup** in a **matrix over every store** and posts **one** PR comment with preview/customize links per store (via `preview-fragment-*` artifacts). If the source branch **is** `staging-<alias>` or `live-<alias>`, only that store is used. Before `shopify theme share`, each publish job overlays that store’s JSON from `stores/<alias>/` onto the repo root (`config/`, `templates/`, `sections/` — same rules as `climaybe switch` / `stores-to-root`), so previews use the correct per-store template JSON rather than whatever happens to be at root on the PR branch. The PR comment is **upserted** (single bot comment) and older bot preview comments are removed. **`reusable-cleanup-themes`:** `cleanup_mode: by_pr` deletes themes whose name ends with `-PR{padded}` for the current PR (used before re-publishing). `cleanup_mode: orphan_pr` lists themes ending with `-PR<n>`, compares `n` to **open** PRs via `gh pr list` (limit 1000), and deletes themes whose PR is not open — used by **`cleanup-orphan-preview-themes.yml`** (on PR close, weekly, and `workflow_dispatch`). **`pr-close.yml`:** uses the same store matrix rule as `pr-update` and deletes this PR’s preview themes on each store; a follow-up job sums **deleted** counts from small artifacts for the PR comment.
- **Lighthouse Shopify auth:** Prefer Dev Dashboard app credentials (`SHOP_CLIENT_ID` + `SHOP_CLIENT_SECRET`) with Admin API scopes `read_products` and `write_themes`, installed on the CI store. The action uses the client credentials grant so you no longer need a static Admin API access token from Admin → Develop apps (new Admin custom apps are not available after Jan 2026). Legacy `SHOP_ACCESS_TOKEN` still works for older Admin-created apps. Also required: `SHOPIFY_STORE_URL` and `LHCI_GITHUB_APP_TOKEN`. When auth or those secrets are missing, Lighthouse skips gracefully.
